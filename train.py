...
from src.mixloss import Tripletloss, mix_loss
ms.set_seed(1)
...
def set_parameter():
    ...
    if config.mode_name == 'GRAPH':
        if target == "Ascend":
            ...
            ms.context.set_context(mode=context.GRAPH_MODE, device_target=target, save_graphs=config.save_graphs,
                           save_graphs_path=rank_save_graphs_path)
        else:
            ms.context.set_context(mode=context.GRAPH_MODE, device_target=target, save_graphs=config.save_graphs)
        set_graph_kernel_context(target, config.net_name)
    else:
        ms.context.set_context(mode=context.PYNATIVE_MODE, device_target=target, save_graphs=False)

    if config.parameter_server:
        ms.context.set_ps_context(enable_ps=True)
    if config.run_distribute:
        if target == "Ascend":
            device_id = int(os.getenv('DEVICE_ID'))
            ms.context.set_context(device_id=device_id)
            ms.context.set_auto_parallel_context(device_num=config.device_num, parallel_mode="data_parallel",
                                         gradients_mean=True)
            set_algo_parameters(elementwise_op_strategy_follow=True)
            if config.net_name == "resnet50" or config.net_name == "se-resnet50":
                if config.boost_mode not in ["O1", "O2"]:
                    ms.context.set_auto_parallel_context(all_reduce_fusion_config=config.all_reduce_fusion_config)
        ...
        # GPU target
            ...
            init()
            ms.context.set_auto_parallel_context(device_num=get_device_num(),
...
def train_net():
    ...
    if config.optimizer == "Thor" and config.dataset == "imagenet2012":
        ...
        opt = nn.thor(net, lr, ms.Tensor(damping), config.momentum, config.weight_decay, config.loss_scale,
                      config.batch_size, frequency=config.frequency)
    ...
    model.train(45, dataset, callbacks=cb,
                sink_size=-1, dataset_sink_mode=True)

    if config.run_eval and config.enable_cache:
        print("Remember to shut down the cache server via \"cache_admin --stop\"")
    mox.file.copy_parallel(src_url=ckpt_save_dir, dst_url=config.train_url)	
